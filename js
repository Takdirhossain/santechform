...
const handalerazorpay = formData => {
  const amount = 120
  axios.post("http://localhost:5000/order", amount)
        .then((res) => {
  ...
  const handalesubmit = (e) => {
    ...
    if (captcha) {
      ...
      Swal.fire({
        ...
        didOpen: () => {
          document.getElementById('razorpay').addEventListener('click', () => handalerazorpay(formData));
          document.getElementById('stripe').addEventListener('click', () => handalestripe(formData));
          document.getElementById('paypal').addEventListener('click', () => handalepaypal(formData));
        },
      });
